...
//check if user selected city
	...
	if(isset($_SESSION['selected_city'])){
		...
		INNER JOIN restaurant_info ON restaurant_info.restaurant_id = products.restaurant_id WHERE restaurant_info.city_id = '$city_id' AND products.availability = 1 ORDER BY product_id DESC LIMIT 16";
		$productsLoad = $operation->retrieveMany($sql);
    	$countProducts = $operation->countAll($sql);
	}else{
		$sql = "SELECT product_id,product_name,product_price, prep_mins, delivery_fee,products.img_url as prod_img_url, products.restaurant_id as restaurant_id, city_id, restaurant_name, restaurant_phone, restaurant_info.img_url as rest_img FROM products 
		INNER JOIN restaurant_info ON restaurant_info.restaurant_id = products.restaurant_id WHERE products.availability = 1 ORDER BY product_id DESC LIMIT 16";
		$productsLoad = $operation->retrieveMany($sql);
    	$countProducts = $operation->countAll($sql);
	}
...
//									display city viewed from
										if(isset($_SESSION['selected_city'])){
											...
											echo $getCity['city_name']." - Meals";
										}else{
											echo "All Cities - Meals";
										}
									?>
