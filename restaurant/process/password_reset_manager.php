...
// print_r($_POST);die();
 if(isset($_POST['reset_email']) && !empty($_POST['reset_email'])){
     ...
     //check email
     $countEmail = $operation->countAll("SELECT * FROM `users` WHERE email = '$email' AND user_role = 'manager'");
     if($countEmail > 0 ){
    ...
    //check the code sent
     ...
     //check email
     $getEmail = $operation->retrieveSingle("SELECT * FROM `password_reset` WHERE email = '$email' AND role='manager' ORDER BY id DESC");
