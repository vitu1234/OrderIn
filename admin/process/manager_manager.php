...
//add driver
if(!empty($_POST['fname']) && !empty($_POST['lname']) && !empty($_POST['email']) && !empty($_POST['phone']) && !empty($_POST['password']) && !empty($_POST['city_name']) && !empty($_POST['rest_name']) && isset($_POST['fname']) && isset($_POST['lname']) && isset($_POST['email']) && isset($_POST['phone']) && isset($_POST['password']) && isset($_POST['city_name']) && isset($_POST['rest_name'])){
    ...
    $rest_id = addslashes($_POST['rest_name']);
    $role = "manager";
  ...
  $subject = "ORDERIN RESTAURANTS";
    ...
    if($operation->countAll("SELECT *FROM users WHERE email = '$email' AND user_role = '$role'") == 0){
        if($operation->insertData($table,$data) == 1){
            ...
            $output.='<p>Use the following credentials to login into your Ordermw Manager Account, Please Keep the Information confidential</p>';
            $output.='<p>-------------------------------------------------------------</p>';
    ...
    //edit driver
}elseif(!empty($_POST['efname']) && !empty($_POST['elname']) && !empty($_POST['eemail']) && !empty($_POST['ephone'])  && !empty($_POST['ecity_name']) && isset($_POST['efname']) && isset($_POST['elname']) && isset($_POST['eemail']) && isset($_POST['ephone'])  && isset($_POST['ecity_name']) && isset($_POST['driver_id']) && !empty($_POST['driver_id']) && isset($_POST['erest_name']) && !empty($_POST['erest_name'])){
    ...
    $erest_name = addslashes($_POST['erest_name']);
    $role = 'manager';
...
//    change account status
    ...
    if($operation->updateData($table,$data,$where) == 1){
        echo json_encode(array("code"=>1,"msg"=>"Manager account ".$response."d","btnMessage"=>"$response"));
    }else{
        echo json_encode(array("code"=>2,"msg"=>"An error occured, please try again later"));
    }
}
