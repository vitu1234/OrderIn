...
if($admin_level == 2){
  ...
  //get selected manager values
      $getMana = $operation->retrieveSingle("SELECT * FROM `restaurant_managers` WHERE user_id = '$id'");
      
  
    //get restaurants
    $getRest = $operation->retrieveMany("SELECT * FROM `restaurant_info`");
    foreach($getRest as $city){
      $slecte = '';
      
      
      if($getMana['restaurant_id'] == $city['restaurant_id']){
        $slecte = 'selected';
      }
      $cit = $city['city_id'];
        $getCity = $operation->retrieveSingle("SELECT * FROM `cities` WHERE city_id = '$cit'");
      
      
        $selectRest .= '<option '.$slecte.' value="'.$city['restaurant_id'].'">'.$city['restaurant_name'].'-'.$getCity['city_name'].'</option>';
    }
}else{
  //identify admin city
  //get city
  ...
  //get selected manager values
    ...
    //get restaurants
    $getRest = $operation->retrieveMany("SELECT * FROM `restaurant_info` WHERE city_id = '$city_id'");
    foreach($getRest as $city){
      $slecte = '';
        ...
        <div class="row">
          ...
          <div class="col-12 col-sm-6">
            <div class="form-group">
              ...
              <input type="text" maxlength="12" onkeypress="return isNumberKey(event)" class="form-control" data-bv-field="fullName" id="ephone" name="ephone" value="0<?=$getDriver['phone']?>" required placeholder="Phone" />
            </div>
          </div>
          ...
          <div class="col-12 col-sm-6">
            <div class="form-group">
              ...
              <select class="form-control" name="erest_name" id="erest_name" required>
                <?=$selectRest?>
              </select>
            </div>
          </div>
          ...
          <div class="col-12  mt-3">
            <div class="form-group">
              ...
              <input type="hidden" class="form-control" data-bv-field="fullName"  name="eemail" required placeholder="Email" value="<?=$getDriver['email']?>" />
                ...
                <button id="btn_save_restaurant" onclick="editManager()" class="btn btn-sm btn-warning text-light" type="submit"><i class="fas fa-save"></i> Save</button>
        ...
        <div id="deleteRestaurantResponse"></div>
          <small class="text-danger mx-3">Are sure you want to delete? Deleted data is unrecoverable</small>
           ...
           <div class="modal-footer">
            ...
            <button id="ebtn_delete_restaurant" onclick="deleteManager('<?=$id?>')" class="mx-2 btn btn-sm btn-danger text-light" type="submit"><i class="fas fa-trash"></i> Yes</button>
        </div>
        ...
        <div id="statusRestaurantResponse"></div>
          <small class="text-danger mx-3">Are sure you want to <?=$msg?> account? </small>
           ...
           <div class="modal-footer">
            ...
            <button class="btn btn-sm btn-secondary text-light" type="button" data-dismiss="modal">No</button>
            <button id="ebtn_status_restaurant" onclick="accountStatusManager('<?=$id?>')" class="mx-2 btn btn-sm btn-warning text-light" type="button"><i class=""></i> Yes</button>
        </div>
