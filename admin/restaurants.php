		...
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					...
					<h3 id="page_title1">All Restaurants</h3>
					</div>
				</div>
				<div class="col-md-6">
					<div class="right-title-text">  
						<ul>
							...
							<li id="page_title2" class="breadcrumb-item active" aria-current="page">All Restaurants</li>
						</ul>
					</div>
				</div>
			</div>
	...
	<section class="how-to-work">
		<div class="container">
          ...
          <div class="row">
              <div class="col-lg-12"> 
				  <?php
//				  	only super admin can add restaurant
				  if($admin_level == 2){
						?>
				   		<a id="btn_to_add" href="#add_restaurant" style="float: right;" class="upload-btn btn-link text-light"><i class="fa fa-plus" ></i> Add</a>
				  		<?php
				  }else{
									...
									//1 is just and admin, 2 is super admin
										if($admin_level == 2){
											...
											foreach($orders as $order){
												...
												$restaurant_city  = $order['city_id'];
												$city_name = $operation->retrieveSingle("SELECT * FROM `cities` WHERE city_id = '$restaurant_city'");
												
												?>
												<tr class="rr">
													<td><img height="50px" src="../images/<?=$order['img_url']?>" /></td>
													<td><?=$order['restaurant_name']?></td>
													<td ><?=$city_name['city_name']?></td>
													<td class="">
													  <a href="#edit_restaurant" onclick="setValues('<?=$order['restaurant_id']?>','<?=$user_id?>')" >&#8226;&#8226;&#8226;</a>
													</td>
												</tr>
												<?php
											}
										}else{
											$query  = "SELECT * FROM `restaurant_info` WHERE city_id = '$city_id'";
											$orders = $operation->retrieveMany($query);
											foreach($orders as $order){
												//get restaurant information
												$restaurant_city  = $order['city_id'];
  ...
  <script>
    ...
    function setValues(rest, user){
      ...
      function locationHashChanged() {  
        if (location.hash === "#add_restaurant") {  
           toAddRestaurant();
        }else if(location.hash === "#edit_restaurant"){
            var rest = $("#rest").val();
            var user = $("#user").val();
            setCookie('ppkcookie1',rest,1);
            setCookie('ppkcookie2',user,1);
            toeditRestaurant(rest,user);     
        }else{
