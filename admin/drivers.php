...
-->
		...
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					...
					<h3 id="page_title1">All Drivers</h3>
					</div>
				</div>
				<div class="col-md-6">
					<div class="right-title-text">  
						<ul>
							...
							<li id="page_title2" class="breadcrumb-item active" aria-current="page">All Drivers</li>
						</ul>
					</div>
				</div>
			</div>
	...
	<section class="how-to-work">
		<div class="container">
          ...
          <div class="row">
              <div class="col-lg-12">               
                      <a href="#add_driver" id="btn_to_add" type="button" style="float: right;" class="upload-btn btn-link text-light"><i class="fa fa-plus"></i> Add</a>
                            ...
                            <table width="100%" class="table table-striped table-hover" id="dataTables-example">
                                ...
                                <tbody>
									...
									//1 is just and admin, 2 is super admin
										if($admin_level == 2){
											$query  = "SELECT * FROM `users` WHERE user_role = 'driver'";
											...
											foreach($orders as $order){
												...
												<tr class="rr">
													...
													<td class="">
													  <a href="#edit_driver" onclick="setValues('<?=$dv_id?>','<?=$user_id?>')">&#8226;&#8226;&#8226;</a>
													</td>
												</tr>
												<?php
											}
										}else{
											$query  = "SELECT * FROM `users` WHERE user_role = 'driver' AND city_id = '$city_id'";
											$orders = $operation->retrieveMany($query);
											foreach($orders as $order){
												$dv_id = $order['user_id'];
												//get customer information
  ...
  <script>
    $(document).ready(function() {
      ...
      function locationHashChanged() {  
        if (location.hash === "#add_driver") {  
           toAddDriver()
        }else if(location.hash === "#edit_driver"){
            var rest = $("#rest").val();
            var user = $("#user").val();
            setCookie('ppkcookie1',rest,1);
            setCookie('ppkcookie2',user,1);
            toeditDriver(rest,user);     
        }else{
