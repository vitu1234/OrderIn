...
-->
		...
		<div class="container">
			<div class="row">
				<div class="col-md-6">
					...
					<h3 id="page_title1">All Managers</h3>
					</div>
				</div>
				<div class="col-md-6">
					<div class="right-title-text">  
						<ul>
							...
							<li id="page_title2" class="breadcrumb-item active" aria-current="page">All Managers</li>
						</ul>
					</div>
				</div>
			</div>
	...
	<section class="how-to-work">
		<div class="container">
          ...
          <div class="row">
              <div class="col-lg-12">               
                      <a href="#add_manager" id="btn_to_add" type="button" style="float: right;" class="upload-btn btn-link text-light"><i class="fa fa-plus"></i> Add</a>
									...
									//1 is just and admin, 2 is super admin
										if($admin_level == 2){
											...
											foreach($orders as $order){
												...
												//get restaurant name
                                                $rest = $operation->retrieveSingle("SELECT * FROM `restaurant_managers` WHERE user_id = '$dv_id'");
                                                $rest_id = $rest['restaurant_id'];
                                                $rest_name = $operation->retrieveSingle("SELECT * FROM `restaurant_info` WHERE restaurant_id = '$rest_id'");
                                                
                                              
												?>
												<tr class="rr">
													<td><?=$order['fname']." ".$order['lname']?></td>
													<td ><?=$rest_name['restaurant_name']?></td>
													<td ><?=$city_name['city_name']?></td>
													<td class="">
													  <a href="#edit_manager" onclick="setValues('<?=$dv_id?>','<?=$user_id?>')">&#8226;&#8226;&#8226;</a>
													</td>
												</tr>
												<?php
											}
										}else{
											$query  = "SELECT * FROM `users` WHERE user_role = 'manager' AND city_id = '$city_id'";
											$orders = $operation->retrieveMany($query);
											foreach($orders as $order){
												$dv_id = $order['user_id'];
												//get customer information
  ...
  <script>
    $(document).ready(function() {
      ...
      function locationHashChanged() {  
        if (location.hash === "#add_manager") {  
           toAddManager()
        }else if(location.hash === "#edit_manager"){
            var rest = $("#rest").val();
            var user = $("#user").val();
            setCookie('ppkcookie1',rest,1);
            setCookie('ppkcookie2',user,1);
            toeditManager(rest,user);     
        }else{
